  ...
  const handleSubmit = (e) => {
    ...
    !search_params.endsWith("?")
      ? path.endsWith("info")
        ...
        ? navigate(`complaints${search_params}`)
        : null
      : null;
  };
    ...
    return (
      <div className={formStyle.filters_wrapper}>
        <div className={buttonsStyle.buttons_container}>
          ...
          <button onClick={handlePopUp} className={buttonStyles.popup_button}>
            {popup.search(/popup/) > 0 ? "˅" : "˄"}
          </button>
        </div>
        <div className={popup}>
          ...
          <div className={buttonsStyle.buttons_container}>
            <button
              form='filter'
              className={buttonsStyle.button + " " + buttonsStyle.small}
            >
              Отфильтровать
            </button>
            <a
