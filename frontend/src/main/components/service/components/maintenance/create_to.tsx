...
import {IHandbook, IMachine, IMaintenance} from "../../../../../configs/intarfaces.ts";
...
export default function CreateTO(){
    ...
    const [create, {isSuccess: createSuccess}] = maintenanceApi.useCreateMaintenanceMutation()
        ...
        <>
            <form className={formStyle.form} onSubmit={handleCreate}>
                                    ...
                                    <section className={formStyle.form_section} key={data[0]}>
                                        ...
                                        <select name={data[0]}>
                                            {handbooks[data[0]] && handbooks[data[0]].results.map((data: IMachine) =>
                                                <option value={data.machine_serial_number}
                                                        key={data.id}>{data.machine_serial_number}</option>
                                            )}
                                        </select>
                                    </section> :
                ...
                <div className={buttonStyle.buttons_container}>
                    <button className={buttonStyle.button} type="submit">Создать ТО</button>
