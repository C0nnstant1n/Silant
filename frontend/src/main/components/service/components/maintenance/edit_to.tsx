...
export default function EditTO(){
    ...
    const [update, {isSuccess: createSuccess}] = maintenanceApi.useUpdateMaintenanceMutation()
    ...
    const handleUpdate = async (e) => {
        ...
        maintenance['id'] = path
        if (TO){maintenance['machine'] = TO.machine}
        // console.log(machine)
        ...
        <>
            <form className={formStyle.form} onSubmit={handleUpdate}>
                {content.map((detail) =>
                        <div className={styles.detail_wrapper} key={detail[0]}>
                            <section className={formStyle.form_section}>
                                ...
                                {detail[0] == 'machine' ? <p><b>{detail[1]}</b></p>:
                                    detail[1].length || typeof(detail[1]) == 'number' ?
                                        <input required={detail[0] != 'machine'}
                                               name={detail[0]} defaultValue={detail[1]}
                                               type={detail[0].search('date') >= 0 ? 'date' :
                                                   detail[0] == 'operating_time' ? 'number' : 'text'}/>
                                        :
                ...
                <div className={buttonStyle.buttons_container}>
                    <button className={buttonStyle.button} type="submit">Сохранить ТО</button>
                    <Link to={'/service/to'} className={buttonStyle.button}>Отмена</Link>
                </div>
            </form>
