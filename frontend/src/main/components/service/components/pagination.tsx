...
export default function Pagination({ response }: IProps) {
  ...
  if (response) {
    ...
    response.previous
      ? (previous = response.previous.match(pattern))
      : (previous = null);
  }
  ...
  return (
    <div className={buttonStyle.buttons_container}>
      {previous ? (
        <Link to={location.pathname + previous[0]}>
          <img
            className={buttonStyle.arrow_up}
            src={arrow}
            alt='Показать предыдущие'
          />
        </Link>
      ) : null}
      {next ? (
        <Link to={location.pathname + next[0]}>
          <img
            className={buttonStyle.arrow_down}
            src={arrow}
            alt='Показать следущие'
          />
        </Link>
      ) : null}
    </div>
