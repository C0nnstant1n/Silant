...
export default function ComplaintDetail(){
    ...
    const [remove ,{isSuccess: deleteSuccess}] = complaintsApi.useDeleteComplaintsMutation()
        ...
        {(
            ...
            :
                ...
                <div className={styles.detail}>
                    <div className={styles.detail}>
                        {complaint ? complaint.map((data: string[]) =>
                            <Detail data={data} key={data[0]}/>
                        ) : null}
                    </div>

                    <div className={buttonsStyles.buttons_container}>
                        <button className={buttonsStyles.button} onClick={handleEdit}>Изменить рекламацию
                        </button>
                        <button className={buttonsStyles.button} onClick={handleDelete}>Удалить рекламацию
                        </button>
                    </div>
