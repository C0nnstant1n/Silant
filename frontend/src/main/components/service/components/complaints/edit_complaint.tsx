...
export default function EditComplaint (){
    ...
    const handleUpdate = async (e) => {
        ...
        complaint['id'] = path
        if (data) {complaint['machine'] = data.machine.id}
        ...
        <>
            <form className={formStyle.form} onSubmit={handleUpdate}>
                {content.map((detail) =>
                    ...
                    <div className={styles.detail_wrapper} key={detail[0]}>
                        <section className={formStyle.form_section}>
                            ...
                            {detail[0] == 'machine' ? <p><b>{detail[1].machine_serial_number}</b></p>:
                ...
                <div className={buttonStyle.buttons_container}>
                    <button className={buttonStyle.button} type='submit'>Сохранить</button>
                    <Link to={'/service/complaints'} className={buttonStyle.button}>Отмена</Link>
                </div>
            </form>
            <div style={{display: "none"}}>{isSuccess ?
